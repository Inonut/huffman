...
public final class AdaptiveHuffmanDecompress {
	...
	public static void main(String[] args) throws IOException {
		...
		File inputFile = new File(path + "testMusic.huff_png");
		File outputFile = new File( path + "testMusic2.mp3");
	...
	static void decompress(BitInputStream inputStreamn, OutputStream out) throws IOException {
		int[] initFreqs = new int[257];
		Arrays.fill(initFreqs, 1);
		
		FrequencyTable freqTable = new FrequencyTable(initFreqs);
		HuffmanDecoder dec = new HuffmanDecoder(inputStreamn);
